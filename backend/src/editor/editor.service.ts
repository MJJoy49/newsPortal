...
export class EditorService {
  createNewsAndPublish(createNewsDto: CreateNewsDto): object {
    return { message: "This action will create a news", title: createNewsDto.title, breaking: createNewsDto.isBreaking };
  }
  showNewsList(status?: string, categoryId?: number, page?: number, limit?: number): object {
    return { message: 'This action will show news list', status: status, categoryId: categoryId, page: page, limit: limit }
  }
  ...
  updateNews(id: string, updateNewsDto: UpdateNewsDto): object {
    return {message: `this action will update news ${id}, ${updateNewsDto.title}`}
  }
